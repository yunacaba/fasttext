...
bool comparePairs(const std::pair<real, std::string> &l, const std::pair<real, std::string> &r);
...
FastText::FastText() : quant_(false), wordVectors_(nullptr), trainException_(nullptr)
{
}
...
void FastText::setMatrices(const std::shared_ptr<DenseMatrix> &inputMatrix,
...
void FastText::saveOutput(const std::string &filename)
{
    ...
    if (quant_)
    {
        throw std::invalid_argument("Option -saveOutput is not supported for quantized models.");
    }
...
void FastText::quantize(const Args &qargs, const TrainCallback &callback)
{
    ...
    if (qargs.cutoff > 0 && qargs.cutoff < input->size(0))
    {
        ...
        dict_->prune(idx);
        std::shared_ptr<DenseMatrix> ninput = std::make_shared<DenseMatrix>(idx.size(), args_->dim);
    ...
    input_ = std::make_shared<QuantMatrix>(std::move(*(input.get())), qargs.dsub, qargs.qnorm);

    if (args_->qout)
    {
        output_ = std::make_shared<QuantMatrix>(std::move(*(output.get())), 2, qargs.qnorm);
    }
    quant_ = true;
    auto loss = createLoss(output_);
    model_ = std::make_shared<Model>(input_, output_, loss, normalizeGradient);
...
void FastText::supervised(Model::State &state, real lr, const std::vector<int32_t> &line,
...
void FastText::skipgram(Model::State &state, real lr, const std::vector<int32_t> &line)
{
    std::uniform_int_distribution<> uniform(1, args_->ws);
    for (int32_t w = 0; w < line.size(); w++)
    {
...
std::tuple<int64_t, double, double> FastText::test(std::istream &in, int32_t k, real threshold)
{
    Meter meter(false);
    test(in, k, threshold, meter);

    return std::tuple<int64_t, double, double>(meter.nexamples(), meter.precision(), meter.recall());
}

void FastText::test(std::istream &in, int32_t k, real threshold, Meter &meter) const
{
...
void FastText::predict(int32_t k, const std::vector<int32_t> &words, Predictions &predictions, real threshold) const
{
    if (words.empty())
    {
        return;
...
FullPrediction FastText::predictFull(int32_t k, std::string_view in, real threshold) const
{
    std::vector<int32_t> words, labels;
    dict_->getStringNoNewline(in, words, labels);
    Predictions linePredictions;
    linePredictions.reserve(k);
    this->predict(k, words, linePredictions, threshold);

    return FullPrediction(std::move(linePredictions), dict_);
}

bool FastText::predictLine(std::istream &in, std::vector<std::pair<real, std::string>> &predictions, int32_t k,
...
std::vector<std::pair<std::string, Vector>> FastText::getNgramVectors(const std::string &word) const
{
...
void FastText::lazyComputeWordVectors()
{
    if (!wordVectors_)
    {
        wordVectors_ = std::unique_ptr<DenseMatrix>(new DenseMatrix(dict_->nwords(), args_->dim));
        precomputeWordVectors(*wordVectors_);
    }
}

std::vector<std::pair<real, std::string>> FastText::getNN(const std::string &word, int32_t k)
{
...
std::vector<std::pair<real, std::string>> FastText::getNN(const DenseMatrix &wordVectors, const Vector &query,
...
std::vector<std::pair<real, std::string>> FastText::getAnalogies(int32_t k, const std::string &wordA,
                                                                 const std::string &wordB, const std::string &wordC)
{
    Vector query(args_->dim);
    query.zero();
