...
class ElapsedTimeMarker
{
  ...
  public:
    ...
    double getElapsed()
    {
        return fasttext::utils::getDuration(start_, std::chrono::steady_clock::now());
    }
};
...
template <typename T>
T getArgGauss(T val, std::minstd_rand &rng, double startSigma, double endSigma, double t, bool linear)
{
    T returnValue;
    const double stddev = startSigma - ((startSigma - endSigma) / 0.5) * std::min(0.5, std::max((t - 0.25), 0.0));
...
template <typename T>
T updateArgGauss(T val, T min, T max, double startSigma, double endSigma, double t, bool linear, std::minstd_rand &rng)
{
...
AutotuneStrategy::AutotuneStrategy(const Args &originalArgs, std::minstd_rand::result_type seed)
    : bestArgs_(), maxDuration_(originalArgs.autotuneDuration), rng_(seed), trials_(0), bestMinnIndex_(0),
      bestDsubExponent_(1), bestNonzeroBucket_(2000000), originalBucket_(originalArgs.bucket)
{
    minnChoices_ = {0, 2, 3};
...
Args AutotuneStrategy::ask(double elapsed)
{
    ...
    {
        args.wordNgrams = updateArgGauss(args.wordNgrams, 1, 5, 4.3, 2.4, t, true, rng_);
    }
    if (!args.isManual("dsub"))
    {
        int dsubExponent = updateArgGauss(bestDsubExponent_, 1, 4, 2.0, 1.0, t, true, rng_);
        args.dsub = (1 << dsubExponent);
    }
    if (!args.isManual("minn"))
    {
        int minnIndex =
            updateArgGauss(bestMinnIndex_, 0, static_cast<int>(minnChoices_.size() - 1), 4.0, 1.4, t, true, rng_);
    ...
    {
        int nonZeroBucket = updateArgGauss(bestNonzeroBucket_, 10000, 10000000, 2.0, 1.5, t, false, rng_);
...
Autotune::Autotune(const std::shared_ptr<FastText> &fastText)
    : fastText_(fastText), elapsed_(0.), bestScore_(0.), trials_(0), sizeConstraintFailed_(0), continueTraining_(false),
      strategy_(), timer_()
{
}
...
void Autotune::printInfo(double maxDuration)
{
    ...
    std::cerr << " ETA: " << utils::ClockPrint(std::max(maxDuration - elapsed_, 0.0));
    std::cerr << std::flush;
}

void Autotune::timer(const std::chrono::steady_clock::time_point &start, double maxDuration)
{
    elapsed_ = 0.0;
...
double Autotune::getMetricScore(Meter &meter, const metric_name &metricName, const double metricValue,
...
int Autotune::getCutoffForFileSize(bool qout, bool qnorm, int dsub, int64_t fileSize) const
{
    ...
    if (qout)
    {
        ...
        outModelSize = 21 + (outM * ((outN + 2 - 1) / 2)) + outputPqSize + (qnorm ? outM : 0);
    }
    else
    {
...
bool Autotune::quantize(Args &args, const Args &autotuneArgs)
{
    ...
    args.retrain = true;
    args.cutoff = getCutoffForFileSize(args.qout, args.qnorm, args.dsub, autotuneArgs.getAutotuneModelSize());
...
{
    std::unordered_set<std::string> argsToCheck = {"epoch",  "lr",   "dim",  "wordNgrams", "loss",
                                                   "bucket", "minn", "maxn", "dsub"};
    for (const auto &arg : argsToCheck)
    {
        if (autotuneArgs.isManual(arg))
        {
            std::cerr << "Warning : " << arg << " is manually set to a specific value. "
...
void Autotune::train(const Args &autotuneArgs)
{
    ...
    trainArgs.verbose = 0;
    strategy_ = std::unique_ptr<AutotuneStrategy>(new AutotuneStrategy(trainArgs, autotuneArgs.seed));
    ...
    while (keepTraining(autotuneArgs.autotuneDuration))
    {
        ...
        try
        {
            ...
            if (sizeConstraintOK)
            {
                ...
                fastText_->test(validationFileStream, autotuneArgs.autotunePredictions, 0.0, meter);

                currentScore = getMetricScore(meter, autotuneArgs.getAutotuneMetric(),
                                              autotuneArgs.getAutotuneMetricValue(), metricLabel);
    ...
    if (bestScore_ == kUnknownBestScore)
    {
        ...
        {
            errorMessage = "Couldn't fulfil model size constraint: please increase "
                           "`autotune-modelsize`.";
        }
        else
        {
            errorMessage = "Didn't have enough time to train once: please increase "
                           "`autotune-duration`.";
        }
