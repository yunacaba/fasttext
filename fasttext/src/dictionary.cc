...
Dictionary::Dictionary(std::shared_ptr<Args> args)
    : args_(args), word2int_(MAX_VOCAB_SIZE, -1), size_(0), nwords_(0), nlabels_(0), ntokens_(0), pruneidx_size_(-1)
{
}

Dictionary::Dictionary(std::shared_ptr<Args> args, std::istream &in)
    : args_(args), size_(0), nwords_(0), nlabels_(0), ntokens_(0), pruneidx_size_(-1)
{
    load(in);
}
...
void Dictionary::getSubwords(const std::string &word, std::vector<int32_t> &ngrams,
...
void Dictionary::computeSubwords(const std::string &word, std::vector<int32_t> &ngrams,
                                 std::vector<std::string> *substrings) const
{
...
void Dictionary::addSubwords(std::vector<int32_t> &line, const std::string_view token, int32_t wid) const
{
    if (wid < 0)
    { // out of vocab
        if (token != EOS)
...
int32_t Dictionary::getLine(std::istream &in, std::vector<int32_t> &words, std::vector<int32_t> &labels) const
{
...
int32_t Dictionary::getStringNoNewline(std::string_view in, std::vector<int32_t> &words,
...
std::string_view Dictionary::getLabelRef(int32_t lid) const
{
    if (lid < 0 || lid >= nlabels_)
    {
        throw std::invalid_argument("Label id is out of range [0, " + std::to_string(nlabels_) + "]");
